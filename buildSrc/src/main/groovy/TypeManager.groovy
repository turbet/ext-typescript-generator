class TypeManager
{
	...
	def getExtendsConfig( fileJson, isInterface ) {
		def result = ""

		
//		println "getExtendsConfig " + fileJson.name + " " + isInterface
//		println "fileJson.superclasses = " + fileJson.superclasses
//		println "getImplementedInterfaces( fileJson ) = " + getImplementedInterfaces( fileJson )
//		println "getImplementedInterfacesConfig( fileJson ) = " + getImplementedInterfacesConfig( fileJson )
//		
		...
		if( fileJson.superclasses?.size() > 0 ) {
			if( isInterface ) {
				if( fileJson.superclasses.last().contains( "Ext." ) )
					result = "extends Typ${ normalizeType( convertToInterface( fileJson.superclasses.last() ), true ) }Config"
			}
			else {
				if( getClassName( fileJson.name ) != fileJson.superclasses.last() )
					result = "extends Typ${ normalizeType( fileJson.superclasses.last(), true ) }Config"
			}
		}
		if( isInterface ) {
			def interfaces = getImplementedInterfacesConfig( fileJson )
			if( !result.length() && interfaces.length() ) {
				result = "extends "
			}
			else if( result.length() && interfaces.length() ) {
	...
	def getImplementedInterfacesConfig( fileJson ) {
		def result = ""

		if( !config.interfaceOnly )
			result = "Typ" + normalizeType( convertToInterface( fileJson.name ), true ) + "Config"

		def implementedInterfaces = []

		// unique(false) is used to remove duplicated(x2, x3) references in json doc generated by jsduck
		// added by JeT nov 2015
		fileJson.mixins?.unique(false).each { thisMixin ->
			implementedInterfaces.add( "Typ" + normalizeType( convertToInterface( thisMixin ), true ) + "Config" )
		}
